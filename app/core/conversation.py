...
def get_fallback_response(
    ...
    business = (property_name or "").strip() or BUSINESS_NAME
    if state == CallState.WRAP_UP.value:
        closing = (
            closing_message.replace("{property_name}", business)
            if closing_message
            else (
                "Thank you. A leasing specialist will review your information "
                "and follow up soon."
    ...
    q_cfg = questions_index(questions).get(state)
    if q_cfg:
        return {
            "response_text": q_cfg.get("retry_prompt") or q_cfg.get("question", ""),
            "understood": False,
            "question_complete": False,
            "extracted_data": {},
            "next_state": state,
    ...
    return {
        "response_text": f"Thank you for calling {business}. Goodbye.",
        "understood": True,
        "extracted_data": {},
        "next_state": "ENDED",
        "call_complete": True,
